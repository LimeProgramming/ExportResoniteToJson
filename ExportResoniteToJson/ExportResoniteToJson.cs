...
public class ExportResoniteToJson : ResoniteMod
{
    ...
    public override string Link => "https://github.com/LimeProgramming/ExportResoniteToJson";
    ...
    [AutoRegisterConfigKey]
    private static readonly ModConfigurationKey<bool> EXPORT_BRSON = new ModConfigurationKey<bool>(
        "Export BRSON Type", "Enable exprting to Brson file type. Changing this setting requires a game restart to apply.",
        () => false
    );

    [AutoRegisterConfigKey]
    private static readonly ModConfigurationKey<bool> EXPORT_7ZBSON = new ModConfigurationKey<bool>(
        "Export 7ZBSON Type", "Enable exprting to 7zbson file type. Changing this setting requires a game restart to apply.",
        () => false
    );
    ...
    public override void OnEngineInit()
    {
        ...
        if (Config.GetValue(EXPORT_BRSON))
        {
        ...
        if ((Config.GetValue(EXPORT_JSON)) || (Config.GetValue(EXPORT_BRSON)) || (Config.GetValue(EXPORT_7ZBSON)) || !(Config.GetValue(EXPORT_LZ4BSON)))
        {
        ...
        MethodInfo exportModelOriginal = AccessTools.DeclaredMethod(typeof(ModelExporter), nameof(ModelExporter.ExportModel), new Type[] {
            typeof(Slot),
            typeof(string),
            typeof(Predicate<FrooxEngine.Component>)
        });
    ...
    private static bool ExportModelPrefix(Slot slot, string targetFile, Predicate<FrooxEngine.Component> filter, ref Task<bool> __result)
    {
